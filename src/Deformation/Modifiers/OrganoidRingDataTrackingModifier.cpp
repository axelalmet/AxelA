...
OrganoidRingDataTrackingModifier<DIM>::OrganoidRingDataTrackingModifier()
...
template<unsigned DIM>
void OrganoidRingDataTrackingModifier<DIM>::UpdateAtEndOfTimeStep(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
//    UpdateCellData(rCellPopulation);
//
//    CalculateModifierData(rCellPopulation);
//
//    *mpOrganoidRingDataFile << "\n";

}

template<unsigned DIM>
void OrganoidRingDataTrackingModifier<DIM>::SetupSolve(AbstractCellPopulation<DIM,DIM>& rCellPopulation, std::string outputDirectory)
{
    /*
    ...
    mpOrganoidRingDataFile = output_file_handler.OpenOutputFile("organoidringdata.dat");
...
template<unsigned DIM>
void OrganoidRingDataTrackingModifier<DIM>::UpdateCellData(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
...
//Overridden method to close file at the end of solve---note that we only need the data at the start and end of simulations
template<unsigned DIM>
void OrganoidRingDataTrackingModifier<DIM>::UpdateAtEndOfSolve(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
    UpdateCellData(rCellPopulation);
	...
	// Close output file.
    mpOrganoidRingDataFile->close();
}

//Method to calculate the volume of all the cells and Matrigel
template<unsigned DIM>
double OrganoidRingDataTrackingModifier<DIM>::CalculateTotalVolume(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
...
//	double unit_gel_volume = 0.0;
	...
	else if (dynamic_cast<MeshBasedCellPopulation<DIM>*>(&rCellPopulation))
	{
		...
		//Multiply the dimension lengths together
		for (unsigned i = 0; i<DIM; i++)
		{
			volume_of_tissue *= domain_width[i];
		}
	...
	else if (dynamic_cast<NodeBasedCellPopulation<DIM>*>(&rCellPopulation))
	{
		...
		//Multiply the dimensino lengths ogether
		for (unsigned i = 0; i<DIM; i++)
		{
			volume_of_tissue *= domain_width[i];
		}
...
template<unsigned DIM>
double OrganoidRingDataTrackingModifier<DIM>::CalculateRingVolume(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
	//Initialise area
	...
	area_of_ring = 0.5*std::abs(area_of_ring);
...
// not do.e
template<unsigned DIM>
c_vector<double,DIM> OrganoidRingDataTrackingModifier<DIM>::CalculateCellPopulationWidth(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
	//Initialise vectors
	...
	for (unsigned i = 0; i < DIM; i++)
	{
		domain_width[i] = std::abs(maximum_points[i] - minimum_points[i]);
	}
...
template<unsigned DIM>
double OrganoidRingDataTrackingModifier<DIM>::CalculateRingPerimeter(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
	...
	else if (dynamic_cast<NodeBasedCellPopulation<DIM>*>(&rCellPopulation))
	{
		...
		for (unsigned i = 0; i < (cells_in_ring.size() - 1); i++)
		{
			...
			double distance_between_nodes = p_cell_population->rGetMesh().GetDistanceBetweenNodes(first_node_index, second_node_index);
			ring_perimeter += distance_between_nodes;
		}
	}
...
template <unsigned DIM>
std::vector<unsigned> OrganoidRingDataTrackingModifier<DIM>::CountCellProliferativeTypes(AbstractCellPopulation<DIM>& rCellPopulation)
{
...
template<unsigned DIM>
std::vector<unsigned> OrganoidRingDataTrackingModifier<DIM>::CountCellMutationStates(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
	std::vector<unsigned> mutant_cell_count = rCellPopulation.GetCellMutationStateCount();

	//Need to count Paneth cells as well now
	...
	mutant_cell_count.push_back(num_paneth_cells);

	return mutant_cell_count;
}

//Method to get the cells in the epithelial ring in anti-clockwise order
template<unsigned DIM>
std::vector<unsigned> OrganoidRingDataTrackingModifier<DIM>::GetCellsInRingInOrder(AbstractCellPopulation<DIM>& rCellPopulation)
{
...
template<unsigned DIM>
void OrganoidRingDataTrackingModifier<DIM>::CalculateModifierData(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
	...
	//Add the cell mutation state counts
	std::vector<unsigned> mutation_states = CountCellMutationStates(rCellPopulation);

	for (unsigned i = 0; i < mutation_states.size(); i++)
	{
		time_and_data.push_back(mutation_states[i]);
	}


	for (unsigned i = 0; i < time_and_data.size(); i++)
	{
		*mpOrganoidRingDataFile << time_and_data[i] << "\t";
	}
}


template<unsigned DIM>
void OrganoidRingDataTrackingModifier<DIM>::OutputSimulationModifierParameters(out_stream& rParamsFile)
{
...
template class OrganoidRingDataTrackingModifier<1>;
template class OrganoidRingDataTrackingModifier<2>;
//template class OrganoidRingDataTrackingModifier<3>; //3D won't quite work regarding perimeter etc.

// Serialization for Boost >= 1.36
#include "SerializationExportWrapperForCpp.hpp"
EXPORT_TEMPLATE_CLASS_SAME_DIMS(OrganoidRingDataTrackingModifier)
