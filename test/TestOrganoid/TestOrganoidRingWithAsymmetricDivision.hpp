...
#ifndef TESTORGANOIDRINGWITHASYMMETRICDIVISION_HPP_
#define TESTORGANOIDRINGWITHASYMMETRICDIVISION_HPP_
...
#include "StochasticAsymmetricDivisionCellCycleModel.hpp" //Cell cycle model that will randomly assign daughter cells to become Paneth Cells
...
class TestOrganoidRingWithAsymmetricDivision : public AbstractCellBasedTestSuite
{
public:
	void TestTissueRingWithAsymmetricDivision() throw(Exception)
	{
		//Simulation time parameters
		double dt = 0.01; //Set dt
		double end_time = 0.02; //Set end time
...
//		unsigned index = CommandLineArguments::Instance()->GetUnsignedCorrespondingToOption("-myintval");
		...
		//Set index to be 1 since we're not running on the servers.
		unsigned index = 1;
		...
		for (unsigned i = 0; i<real_indices.size(); i++)
		{
			...
			StochasticAsymmetricDivisionCellCycleModel* p_cycle_model = new StochasticAsymmetricDivisionCellCycleModel();
		...
		run << 100*index;
		std::string output_directory = "OrganoidRingWithAsymmetricDivision/" + out.str() + "/" + run.str();
