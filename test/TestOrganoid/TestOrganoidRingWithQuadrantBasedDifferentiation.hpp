/*
 * TestOrganoidRingWithAsymmetricDivision.hpp
 *
...
#ifndef TESTORGANOIDRINGWITHQUADRANTBASEDDIFFERENTIATION_HPP_
#define TESTORGANOIDRINGWITHQUADRANTBASEDDIFFERENTIATION_HPP_
...
#include "StochasticQuadrantBasedCellCycleModel.hpp" //Cell cycle model that will randomly assign daughter cells to become Paneth Cells
...
#include "StemPanethQuadrantTrackingModifier.hpp"

#include "Debug.hpp"

class TestOrganoidRingWithQuadrantBasedDifferentiation : public AbstractCellBasedTestSuite
{
public:
	void TestTissueRingWithQuadrantBasedDifferentiation() throw(Exception)
	{
		...
		//Set the number of bands of alternating cell type that we want
		unsigned num_bands = 8;
...
//
		...
		for (unsigned i = 0; i<real_indices.size(); i++)
		{
			...
			StochasticQuadrantBasedCellCycleModel* p_cycle_model = new StochasticQuadrantBasedCellCycleModel();
		...
		bands << "BANDS_" << num_bands;
		std::stringstream paneth;
		paneth << "PSTIFF_" << paneth_stiffness_multiplier;
		std::stringstream run;
		run << 100*index;
		std::string output_directory = "OrganoidRingWithQuadrantBasedDifferentiation/" + out.str()
				+ "/" + bands.str() + "/" + paneth.str() + "/" + run.str();
		...
		//Add modifier to determine quadrants of stem and paneth cells
		MAKE_PTR(StemPanethQuadrantTrackingModifier<2>, p_quadrant_modifier);
		p_quadrant_modifier->SetNumberOfBands(num_bands);
		simulator.AddSimulationModifier(p_quadrant_modifier);
...
#endif /* TESTORGANOIDRINGWITHQUADRANTBASEDDIFFERENTIATION_HPP_ */
