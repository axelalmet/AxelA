...
#ifndef TESTNODEBASEDBASEMENTMEMBRANEMODEL_HPP_
#define TESTNODEBASEDBASEMENTMEMBRANEMODEL_HPP_
...
class TestNodeBasedBasementMembraneModel : public AbstractCellBasedTestSuite
{
public:
	void TestNodeBoxModel() throw(Exception)
	{
		...
		double target_curvature = 0.0; //Set target curvature
		...
		double left_slough_side =  0.0; //Set this boundary to be negative so that cells don't get sloughed on the left.
		...
		for (double bm = 1.0; bm < 21.0; bm ++ )
		{
			//Define the basement membrane parameter
			double basement_membrane_parameter = 0.0 + 0.25*bm;
...
//				p_cycle_model->SetTransitCellG1Duration(DBL_MAX);
			...
			run << 100*index;
			std::string output_directory = "NodeBasedBoxModel/" + run.str() + "/" + params.str(); //Set the output directory
...
#endif /*TESTNODEBASEDBASEMENTMEMBRANEMODEL_HPP_*/
